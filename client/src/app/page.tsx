    ...
    <div style={{ background: getBg(), minHeight: '100vh', transition: 'background 1s ease' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <h1 style={{ fontSize: '3rem', fontWeight: 800, color: 'white', textShadow: '0 2px 20px rgba(0,0,0,0.3)', margin: 0 }}>
            🌤️ Weather App
          </h1>
        ...
        <SearchBar onSearch={fetchWeather} onLocationSearch={fetchByLocation} darkMode={darkMode} />

        {saved && (
          <div style={{ marginTop: '1rem', padding: '0.8rem 1.5rem', background: 'rgba(34,197,94,0.2)', border: '1px solid rgba(34,197,94,0.4)', borderRadius: '16px', color: 'white', textAlign: 'center' }}>
            ✅ Search saved to database!
          </div>
        )}

        {error && (
          <div style={{ marginTop: '1rem', padding: '1rem 1.5rem', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '16px', color: 'white', textAlign: 'center' }}>
            ⚠️ {error}
          </div>
        )}

        {loading && <LoadingSpinner />}
        {weather && !loading && <WeatherCard weather={weather} darkMode={darkMode} />}
        {forecast && !loading && <ForecastSection forecast={forecast} darkMode={darkMode} />}
        {weather && !loading && <MapEmbed location={weather.name} />}
        {weather && !loading && <YoutubeSection location={weather.name} />}

        {weather && !loading && (
          <div style={{ textAlign: 'center', marginTop: '1.5rem' }}>
            <a href="/saved" style={{ color: 'rgba(255,255,255,0.8)', textDecoration: 'none', fontSize: '0.9rem', border: '1px solid rgba(255,255,255,0.3)', padding: '0.6rem 1.5rem', borderRadius: '50px', background: 'rgba(255,255,255,0.1)' }}>
      ...
      <footer style={{ textAlign: 'center', padding: '2rem', color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem' }}>
        Built by Laraib for PM Accelerator Technical Assessment · Product Manager Accelerator helps aspiring PMs break into product management
      </footer>
    </div>
  );
}